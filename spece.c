...
void printSpriteAt(int x, int y, char *spriteBMP) {
    ...
    {
        spriteShift = 0;
        for (j = 0; j < SPRITE_COLS * SPRITE_SCALE; j += SPRITE SCALE)
        {
...
void updateEnemyPosition(actor *act) {
    if (act->dir == 1) {
        if ((act->horizontalPosition + SPRITE COLS * SPRITE_SCALE + act->speed) < HORIZONTAL_LIM)
...
void printPlayer(actor*play)
{
    ...
    else if (play->dir == 1) //move right
    {
        ...
        play-›horizontalPosition += play-yspeed:
    }
...
bool checkCollision (actor* actl, actor* act2, bool isSecondlaser) {
    ...
    y12 = acti->verticalPosition + SPRITE_ROWS * SPRITE_SCALE;
